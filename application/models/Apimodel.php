...
class Apimodel extends CI_Model {
    ...
    function __construct() {
        ...
        $this->data->set_menu_base($this->data->get_menu_base());
    }
    ...
    public function get_delivery_plan_new($sinput, $grand_total) {
        ...
        $this->requestedData['delivery_postcode'] = $this->session->userdata('delivery_postcode');
        $ch = curl_init();
        curl_setopt_array($ch, array(
    ...
    public function update_profile($data) {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $this->config->item('api_host') . 'api/update_profile',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $data,
            CURLOPT_FOLLOWLOCATION => true
    ...
    public function change_password($email, $pass) {
        $data['CustEmail'] = md5($email);
        $data['CustPassword'] = $pass;
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $this->config->item('api_host') . 'api/change_password',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
    ...
    public function deactivate($email) {
        ...
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $this->config->item('api_host') . 'api/deactivate',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => http_build_query($data),
            CURLOPT_FOLLOWLOCATION => true
    ...
    public function get_orderstatus($MNOID) {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $this->config->item('api_host') . 'api/get_orderstatus/',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => array('OrderId' => $MNOID),
            CURLOPT_FOLLOWLOCATION => true
        ));
        $output = curl_exec($ch);
        curl_close($ch);
