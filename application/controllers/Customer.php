...
class Customer extends Site_Controller {
    ...
    public function profile() {
        ...
        $customer = $this->Customer_Model->get($this->session->userdata('CustId'));
        $this->render_page('customer/profile',$customer);
    }

   public function editprofile() {
        $customer = $this->Customer_Model->get($this->session->userdata('CustId'));
        if ($_POST) {
            $data['CustFirstName'] = $this->input->post('customers_firstname');
            $data['CustLastName'] = $this->input->post('customers_lastname');
            $data['CustEmail'] = $this->input->post('customers_email_address');
            $data['CustTelephone'] = $this->input->post('customers_telephone');
            $data['CustAdd1'] = $this->input->post('customers_address1');
            $data['CustTown'] = $this->input->post('customers_town');
            $data['CustArea'] = $this->input->post('cust_area');
            $data['CustPostcode'] = $this->input->post('customers_postcode') . ' ' . $this->input->post('customers_postcode2');
            $CustEmail = $this->input->post('cust_email_address');
            if ($this->Customer_Model->save($data, $customer->CustId)) {
                $data['CustPrevEmail'] = $CustEmail;
                $this->Apimodel->update_profile($data);
        ...
        $this->body_class[] = 'customer-dashboard edit-profile';
        $this->render_page('customer/editprofile', $customer);
    }
