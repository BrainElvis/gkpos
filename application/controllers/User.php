...
class User extends Site_Controller {
    ...
    function checkout() {
        ...
        foreach ($data['opening_time'] as $index => $row) {
            ...
            $data["order_start"] = ( $order_time[$index]['start'] <= time() && $order_time[$index]['end'] >= time() ) ? $order_time[$index]['start'] : /*$data["order_start"]*/"00:00";
        }
        $order_times = array();
        $today = date('d');
    ...
    function userpromocode() {
         //debugPrint($this->session->userdata('rest_vouchers'));
        if ($this->session->userdata('rest_vouchers')) {
            ...
            if (!empty($myVoucher)) {
                ...
                $price = $myVoucher->price_func == 'pound' ? $myVoucher->price : $myVoucher->price / 100;
                $session_data = array(
            ...
            if ($this->form_validation->run('customer_registration') == FALSE) {
                ...
                if ($this->User_Model->save($data)) {
                    ...
                    $pattern = array(
                        ...
                        '/SITE_LINK/'=>$this->config->item('website')!=''?$this->config->item('website'):$this->config->item('company')
                    );
                    ...
                    //$body_txt = str_replace('BODY_TEXT', nl2br($body_txt), $email_template);
                      $restaurantlogo='<img  src="'.UPLOAD_PATH.$this->config->item('company_logo').'" width="200" height="150" alt="'.$this->config->item('company').'" />';
                      $body_txt = str_replace(array('LOGO', 'BODY_TEXT'), array($restaurantlogo, nl2br($body_txt)), $email_template);
    ...
    function forgotpass() {
        ...
        $this->current_section = "Forgot Password";
        $this->body_class[] = 'customer-forgot-pass';
        $data = [];
        $data['error'] = $error = 0;
        if ($this->input->post('u_name') != null) {
            $uname = $this->input->post('u_name');
            $info = $this->User_Model->get_by(array('CustEmail' => $uname));
            if (count($info) < 1) {
                $data['error'] = $error = 1;
                ...
                $remote = $this->Apimodel->deactivate($info[0]->CustEmail);
                $this->load->model("Notification");
                $email_template = $this->Notification->get_email_template();
                $body_txt = '';
                $body_txt .= 'Dear ';
                ...
                $body_txt .= "If this was you, you can set a new password  " . anchor('user/resetPassword/' . md5($this->input->post("u_name")), 'here') . ".<br /><br />";
                $body_txt .= anchor('user/resetPassword/' . md5($this->input->post("u_name")), 'Reset password');
                ...
                $this->email->from($this->config->item('email'), $this->config->item('company'));
                $this->email->to($uname);
                $this->email->subject($this->config->item('company') . ' ' . $this->lang->line('pass_subject_mess1'));
                $this->email->message($body_txt);
                $this->email->send();
                $this->session->set_userdata(array(
                    'forgotMessage' => '<strong class="successMessage">Account reseting mail has been sent to your mail account. please check</strong>'
                ));
                $data['error'] = $error = 0;
            }
        }
      ...
      function resetPassword($encoded_email = FALSE, $msg = Null) {
        $this->page_title = "Reset Password";
        $this->current_section = "Reset Password";
        $this->body_class[] = 'account-activation';
        $userEmail = $encoded_email;
        $data['password_change_succ'] = $msg;
        $data['encoded_email'] = $encoded_email;
        if ($userEmail) {
            $email = array(
                'md5(CustEmail)' => $userEmail
            );
            $customer = $this->db->get_where('customers', $email)->row();
            if ($customer->CustStatus == 0) {
                $result = FALSE;
                $remote = False;
                if ($_POST) {
                    $result = $this->db->update('customers', array('CustStatus' => '1', 'CustPassword' => md5($_POST['password'])), $email);
                    $remote = $this->Apimodel->change_password($customer->CustEmail, md5($_POST['password']));
                }
                if (true == $result && true == $remote) {
                    $msg = "Your password has been reset successfully.";
                    $data['password_change_succ'] = $msg;
                    redirect('user/resetPassword/' . $encoded_email);
                }
                $this->render_page('user/reset_password', $data);
            }
            else {
