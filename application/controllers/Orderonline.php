...
class Orderonline extends Site_Controller {
    ...
    public function index() {
        ...
        $orderonline= $this->Site->get_page('orderonline');
        ...
        if (empty($restinfo) || empty($order_policy)) {
            ...
            if (!empty($apiData->data)) {
                ...
                if (!$this->session->userdata('rest_promotion')) {
                    $this->data->set_rest_promotion($apiData->data->rest_promotion);
                }
                if (!$this->session->userdata('rest_vouchers')) {
                    $this->data->set_rest_vouchers($apiData->data->rest_vouchers);
                }
    ...
    public function get_delivery_area() {
        ...
        if ($this->session->userdata('deliveryarea')) {
            ...
            if ($deliveryarea[0]->DeliveryAreaId) {
                ...
                $this->session->set_userdata('delivery_postcode', $pcode);
