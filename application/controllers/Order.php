...
class Order extends Site_Controller {
    ...
    private function sent_order_mail($orderid, $MNOID = False) {
       ...
       //$body_txt = str_replace('BODY_TEXT', $body_txt, $email_template);
        ...
        $body_txt = str_replace(array('LOGO', 'BODY_TEXT'), array($restaurantlogo, $body_txt), $email_template);        
        $config['wordwrap'] = false;
        $config['mailtype'] = 'html';
        $config['protocol'] = 'sendmail';
        $this->email->initialize($config);
        $this->email->from($this->config->item('email'), $this->config->item('company'));
        $this->email->to($cust_info->CustEmail);
        $this->email->bcc(array('info@munchnow.co.uk',$this->config->item('email')));
    ...
    function confirmatin_status() {
        ...
        $order = $query->row();
        if ($order->Status < 1 || $order->Status == NULL) {
            $remoteOrderStatus = $this->Apimodel->get_orderstatus($order->MNOID);
            if ($remoteOrderStatus->Status >= 1) {
                $this->db->update('customer_order', array('Status' => $remoteOrderStatus->Status,'DeliveryTime'=>$remoteOrderStatus->DeliveryTime), array('MNOID' => $order->MNOID));
            }
        }
