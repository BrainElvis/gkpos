...
class Gkpos_lib
{
      ...
      if(!$itemalreadyinsale || $item_info->is_serialized)
      {
         $item = array(($insertkey) =>
             array(
                 ...
                 'item_tax_percent'      => isset($item_info->item_tax_percent) ? $item_info->item_tax_percent : to_tax_decimals(0.0),
             )
         );
         //add to existing array
         $items+=$item;
   ...
   //[added new function by zaman]
   function get_zaman_payments()
   {
      $payments = $this->get_payments();
      if(array_key_exists('Due', $payments))
      {
         unset($payments['Due']);
      }
      return $payments;
   ...
   function get_zaman_payments_total()
   {
      $subtotal = 0;
      foreach($this->get_zaman_payments() as $payments)
      {
         $subtotal = bcadd($payments['payment_amount'], $subtotal, PRECISION);
      }
   ...
   function get_zaman_amount_due()
   {
      $payment_total = $this->get_zaman_payments_total();
      $sales_total   = $this->get_total();
      return to_currency_no_money(bcsub($sales_total, $payment_total, PRECISION));
   }
