...
class Orders extends Gkpos_Controller {
    ...
    public function addtocart() {
        ...
        if (!empty($dbcart_items)) {
            foreach ($dbcart_items as $line => $item) {
                ...
                if ($sel == $item['selection']) {
                    $exist_in_dbc = true;
                    $this->update_dbc_cart($line, $order_id, $item['id']);
                }
                else {
                    if ($menu == $item['menu']) {
                        $exist_in_dbc = true;
            ...
            if (!empty($items)) {
                foreach ($items as $line => $item) {
                    ...
                    if ($sel == $item['selection']) {
                        ...
                        $this->update_item_cart($line, $item['selection']);
                    }
                    else {
                        if ($menu == $item['menu']) {
                            $existInCuurentCart = true;
                            $this->update_item_cart($line, $item['menu']);
                        }
                    }
                }
            }
        ...
        if (false == $existInDbCart && false == $existInCuurentCart) {
            $this->insert_into_cart($maxLine, $order_id, $category, $menu, $sel);
        }
        ...
        if (isset($item_info->selection) && (int) $item_info->selection > 0) {
            if (isset($item['selection']) && $item_info->selection == $item['selection']) {
                update_dbc_cart($order_id, $category, $menu, $sel, $quantity);
            }
        }
        else {
            if ($item_info->menu == $item['menu']) {
                update_dbc_cart($order_id, $category, $menu, false, $quantity);
            }
        }
        //if item exists in dbc cart update dbc cart 
        //if item exits in cart update in cart 
        //Add item to cart 
        $maxkey = 0;
    ...
    public function insert_into_cart($order_id, $category, $menu, $sel) {
        $item_info = array();
        if ($sel == 'no') {
            $item_info = $this->Orders_Model->get_cart_item($category, $menu);
        }
    ...
    public function update_item_cart($order_id, $category, $menu, $sel = false, $quantity) {
        
    }

    public function update_dbc_cart($order_id, $category, $menu, $sel = false, $quantity) {
